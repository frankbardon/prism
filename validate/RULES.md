...
| `PRISM_SPEC_062` mark.invalid supported | Semantic | `mark.invalid` decides what a mark does with a row null in a scale-bound channel: `"filter"` (the default) drops it, taking its category out of the scale domain too; `"break"` keeps it, so the category holds its slot on the axis and a path splits either side of the gap. Rejects an unknown value, and rejects `"break"` on a mark whose encoder does not consult the draw mask (`line`, `area`, `point`, `bar`, `rule`, `text` do) — accepting it there would silently give the author `"filter"` while their spec says otherwise. Walks layer / concat / facet / repeat children. |
...
| `PRISM_SPEC_051` legend type coherent | Semantic | `legend.type` must name a legend form the channel can produce. A `gradient` is a continuous bar and needs a numeric domain to run between, so it belongs on a `quantitative` channel; a `symbol` legend is a column of category swatches and needs categories to name, so it belongs on a discrete one. Omitting the key always picks the coherent form (`encode.ResolveLegendKind`), so the rule fires only on an explicit override — which would otherwise render an empty legend with nothing to explain why. Walks layer / concat / facet / repeat children. |
| `PRISM_SPEC_052` legend symbol type | Semantic | `legend.symbol_type` must name a shape the swatch emitter draws — the point mark's own `circle` / `square` / `triangle` / `cross` / `diamond` vocabulary (`scene.PointShapes`), because a legend swatch and a point mark go through the same emitter. An unrecognised name is ignored by the encoder and leaves the default square swatch, so it is named here rather than silently dropped. Walks layer / concat / facet / repeat children. |
| `PRISM_SPEC_063` offset mark supported | Semantic | `x_offset` / `y_offset` subdivide a category's band slot so rows sharing that category draw side by side. Only the marks in `offsetCapableMarks` implement that geometry (`bar`); every other band-seated mark — `rect`, `tick`, `boxplot`, `violin`, `heatmap`, `winloss`, `progress`, the spark adornments — fills its slot with a single shape, so there is nothing to divide. The binding is rejected rather than ignored: without the rule those marks reach `CategorySlots` and dodge anyway, which is a chart the spec never asked for. Walks layer / concat / facet / repeat children. |
| `PRISM_SPEC_064` offset axis coherent | Semantic | An offset needs a band to subdivide, and exactly one axis to subdivide it on. Fires when the offset's own position channel resolves to no band scale (an unbound or continuous `x` under `x_offset`, or `y` under `y_offset`), and when **both** `x_offset` and `y_offset` are bound — a mark dodges along one axis only, so the pair describes no geometry and `spec.ResolveOffset` returns nil for it rather than picking a winner. The both-bound half cannot be checked through `ResolveOffset` for that same reason and reads the raw channel pointers. A duplicate `(category, offset)` key is not a validate question — it depends on the executed rows — and is reported at encode time as `PRISM_WARN_OFFSET_COLLISION`. Walks layer / concat / facet / repeat children. |
| `PRISM_SPEC_065` offset excludes explicit stack | Semantic | An explicit `stack` on a position channel may not sit beside a bound offset channel: a stack accumulates the segments along the measure axis and a dodge spreads them across the category band, so the two spend the same geometry on the same grouping. Implicit stacking already steps aside for an offset inside `spec.ResolveStack`, which is why this fires only on a `stack` the author typed — and why the fixup is to remove that key, not the offset. Walks layer / concat / facet / repeat children. |
| `PRISM_SPEC_066` offset excludes same-axis span | Semantic | An offset channel and a span channel (`x2` / `y2`) may not claim the same axis. A span states both ends of the mark along its axis, leaving the band slot there with nothing to subdivide; `rectAxisExtent`'s span branch wins outright, so without the rule the offset is silently dropped. Scoped to the shared axis only — `y2` with `x_offset` is a legal ranged, dodged bar, and so is `x2` with `y_offset`. Walks layer / concat / facet / repeat children. |
| `PRISM_SPEC_067` transform is executable | Semantic | The spec grammar accepts sixteen transform discriminators; not all of them have an implementation behind them. A transform with none decodes, passes shape validation, builds a plan node and then dies inside `plan.Execute` with `PRISM_COMPILE_001` naming an internal node kind the author never wrote — so "validates" and "draws" were independent claims. Rejecting here moves that failure earlier; it creates none. Today the set is `pivot` alone (use `crosstab` for long → wide, or reshape upstream — Prism consumes materialized rows by design). `join` and `union` DO execute, through Execute bodies on the nodes themselves rather than the in-memory backend. Executability means both halves — an executor, and the `SetBackend` wiring a backend-routed node needs — so the table in `validate/rules/transform_executable.go` is cross-checked end to end by `internal/gates/transform_executable_sync_test.go` rather than trusted. Walks layer / concat / facet / repeat children. |
