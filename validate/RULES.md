...
| `PRISM_SPEC_046` mark orient supported | Semantic | A declared `mark.orient` must be a value some mark draws. `bar` / `rect` / `progress` read `vertical` / `horizontal` (which axis carries the category vs the measure); `tree` / `dendrogram` / `network` read the same pair as the direction their layout grows; every other mark type rejects the property instead of ignoring it. `radial` is vocabulary-only — no mark implements it — so it is rejected everywhere. Whether an explicitly requested orientation is *drawable* against the spec's actual scales is an encode-time question (`PRISM_ENCODE_001`), not this rule's. Walks layer / concat / facet / repeat children. |
