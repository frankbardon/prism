...
A discrete `domain` (`band` / `point` / `ordinal`) pins the leading category order and appends any unlisted data category after it — an intentional divergence from Vega-Lite, chosen so a partial domain reorders without dropping rows. `nice` is boolean-only on the wire; the schema rejects Vega-Lite's numeric tick-count form (use `axis.tick_count`). Malformed domains raise `PRISM_SPEC_041` from both `validate/rules/scale_domain.go` and the encoder.
