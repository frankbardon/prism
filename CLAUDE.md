...
- Pulse aggregate aliases mirror Vega-Lite: `count`, `sum`, `mean`, `median`, `min`, `max`, `stdev`, `variance`, `q1`, `q3`, `ci0`, `ci1`. Prism adds `distinct`, `mode`, and the v0.22 distribution-shape scalars `range`, `skewness`, `kurtosis`, `null_count` (`null_count` is universal — any field type; the rest are quantitative/temporal only). Cohort-analytics extensions are `wmean`, `ratio`, `lift`, `share`. `AGG_FREQUENCY` (map-valued) and `AGG_ZSCORE` (degenerate per group — mean z-score is always 0) are intentionally NOT aliased.
