...
| **Mark orientation** (`mark.orient`) — which marks read it, or how a category/measure axis is chosen | `encode/marks/orient.go` — `Orientation` + `MarkOrientation` (the inference rule) + `CategorySlots` / `BaselinePixel` / `MeasureSpans` / `OrientedRect` are the single source of oriented geometry. A baseline-anchored mark must call `MarkOrientation` once and read its geometry through those helpers; it must NOT re-derive "x is the band axis" inline, and it must NOT add a second orientation field (`bullet`'s pre-existing `orientation` is the one documented exception — different default, whole-mark rotation rather than an axis swap). Band slots go through `rectAxisExtent` (`encode/marks/span.go`), the ONE normaliser for a signed band step — a y band scale runs bottom-to-top and its `BandWidth()` is negative, so reading it straight into `RectGeom.H` yields an undrawable rect. Plus `spec/mark.go` (`MarkDef.Orient`), `schema/v1/mark.schema.json` (`orient` enum — it carries the full vocabulary, `radial` included, so `PRISM_SPEC_044` answers with a real message instead of an opaque enum miss), the `orientAwareMarks` table in `validate/rules/mark_orient.go` (a mark that draws no orientation must REJECT `orient` — never ignore it), `errors/codes.go`, `validate/RULES.md`, and the Orientation section of `docs/src/concepts/marks.md` (including the inference table). Stacking (E5-S2) swaps the anchor by replacing `BaselinePixel`, not by branching on the axis again. |
