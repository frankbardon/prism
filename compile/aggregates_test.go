...
// expectedAliases mirrors the 18 friendly aliases the validator
// accepts on quantitative fields (see validate/rules/agg_compat.go).
// Drift between the validator and this map breaks the alias
// passthrough — TestPrismAggOpEnumCoverage catches it.
var expectedAliases = []string{
	"ci0", "ci1", "count", "distinct", "lift", "max", "mean",
	"median", "min", "mode", "q1", "q3", "ratio", "share", "stdev",
	"sum", "variance", "wmean",
}
