...
// tier geometry (see geodata/geometry_host.go), and runCLI drives the
// CLI in-process, so the geoshape plot smoke test needs the loader
// directory configured before any geoshape mark is encoded.
//
// Runtime wiring of the directory from a CLI flag / env var lands in a
// later story; this shim only serves the cmd/prism package's tests.
